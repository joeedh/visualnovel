...
A hundred and forty-one, in twenty namespaces. Seventy-five are `mutating`; eighty-eight declare
a precondition; forty-eight are undoable; seventeen ask for confirmation. (The table below is every
one of them but `notify.*`, which [`../plans/archive/notifications.md`](../plans/archive/notifications.md) states in full.)
...
| `gengraph.create` ✍ ↺ ✓        | `name`                            | Start a graph at `vngen/work/graphs/<slug>.json`, holding one Output node. The slug comes from the name once, at creation, so a graph is renamed the way a scene is — not at all. |
...
| `gengraph.addNode` ✍ ↺ ✓       | `slug`, `type`, `x`, `y`          | Place one node of a registered type. A type no plugin provides is refused by name rather than written and reported on the next load. |
...
| `gengraph.apply` ✍ ↺ ✓         | `slug`, `description`             | Rewrite a whole graph from a JSON description in path.ux's graph DSL, diffed by node id so a node the description leaves alone keeps its position and its journal. The description is a string prop because `@vn/commands` has no JSON kind. |
