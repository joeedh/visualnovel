...
- **A pane shows an editor, and the list of editors is written down once.**
  ...
  document-tree node (see [Documents](#documents)), so an eighteenth editor that forgets to is
...
**Every mutating gesture becomes a `gengraph.*` command.** `renderer/rules/gengraph.ts` is the pure
half: it reads one of path.ux's gesture kinds as a `GenEdit` and names the command that writes it,
and it refuses by name the six kinds this application has no command for. The delegate's `check`
runs `decideGenEdit` locally rather than `stack.check` over IPC, because path.ux's check is
synchronous and runs once per frame per pointer move while `command:check` is an async round trip.
Both sides run the same decision function, so the mid-gesture verdict matches the verdict on
commit. A refused gesture says its refusal, except a refused drag, which path.ux already shows by
