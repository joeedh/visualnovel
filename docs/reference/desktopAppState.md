...
| The image models OpenRouter routes to, with ratios, seed and price | `<userConfigDir()>/models.json`                             | ✓ Survives restart, on this machine                     | `project.info` (both image-model pickers), `hostPriceTables`, the image router         | `models.refresh`, only when the author presses Refresh models                                                       |
