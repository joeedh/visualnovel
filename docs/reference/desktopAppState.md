...
| The image models OpenRouter routes to, with ratios, seed and price, and the text models of OpenRouter, Anthropic and Gemini | `<userConfigDir()>/models.json`                             | ✓ Survives restart, on this machine                     | `project.info` (every model picker), `hostPriceTables`, the image router               | `models.refresh`: when the author presses Refresh models, and once from the Project editor when the file holds no text listing |
| The author's own skills, shared by every project on this machine                                                            | `<userConfigDir()>/skills/<id>/`                            | ✓ Survives restart, on this machine                     | `skillRoots` (the agent's `discover_skills`, `workspace:skilltree`, the document tree) | `skill.cloneToUser`, or the author editing the folder; not undoable, since no snapshot covers it                               |
