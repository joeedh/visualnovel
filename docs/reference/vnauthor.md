...
| Storyboards         | `read_shots`, `set_coverage` **M**, `set_panels` **M**, `propose_storyboard` (costs a model call, writes nothing; follows `shot_form`, and `form` overrides it for one scene when the author asks), `write_storyboard` **M** |
