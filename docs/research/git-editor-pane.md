...
| Need                                                                                                                                        | Where                                                                    | Why                                                                                                                  | As shipped (2026-09-21)                                                                                                                                              |
| ------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Git.log` returning trailers, parents, body, `--numstat`, with `before`, `path` and `author` filters                                        | `packages/git/src/git.ts:214-231`                                        | The list, the maker rule and the per-file history                                                                    | `Git.history(opts)` beside the untouched `Git.log`, with `parsers` in `parse.ts`; `who` is a post-filter in the session                                              |
| `Git.changes(sha)` and `Git.diffPath(sha, path)`                                                                                            | new methods beside `show`/`diff`, `git.ts:245-256`                       | The change view and the per-kind diff                                                                                | As written; `diffPath` takes `against`                                                                                                                               |
| `Git.blob(sha, path)` and a `vngit://` protocol handler                                                                                     | `git.ts`; beside `assetprotocol.ts:29-43`                                | A deleted picture's old side; the other side of a conflict                                                           | As written; `assets/gitprotocol.ts`, and `vngit://<role>/<blobId>.<ext>` by blob id as well as by path                                                               |
| `Git.tag`, `Git.listTags`, `Git.deleteTag` for annotated tags                                                                               | `git.ts:301-320` area                                                    | Checkpoints with notes; `update-ref` makes lightweight tags with no message                                          | `tag`, `checkpoints`, `deleteTag` over `refs/tags/vn/checkpoint/`                                                                                                    |
| `Git.fetch`, `Git.push`, `Git.rebase`, `Git.remotes`, `Git.remoteAdd/Remove/SetUrl`, `Git.upstream`, `Git.setUpstream`, `Git.aheadBehind`   | new; `run` gains `GIT_TERMINAL_PROMPT=0` (`git.ts:20-42`)                | Sync; a prompt for a password must fail rather than hang a hidden subprocess                                         | As written, plus `trackRemote`, `unsent`, `rangeKeys`; `rebase` runs `--autostash` and absorbs a stop over the app's own log writes                                  |
| `Git.inProgress` (rebase, merge, revert), `Git.checkoutSide`, `Git.rebaseContinue`, `Git.rebaseAbort`, `Git.mergeAbort`, `Git.revertDryRun` | new                                                                      | The status and conflict views and the take-back refusal                                                              | `resolveSide` (git's `ours`/`theirs`), `revertAbort`, `revertIntoTree`; `inProgress` reads `orig-head` too                                                           |
| `Committer` skipping a repository with a rebase, merge or revert in progress, and `Git.commit` refusing in that state                       | `packages/commands/src/commit.ts:149-157`; `packages/git/src/git.ts:184` | `add -A` mid-rebase marks conflicted paths resolved, markers and all, and the commit is adopted as the replayed save | `Git.commit` throws `InProgressError`; `Committer.run` catches it and reports through `onSkip`                                                                       |
| `rewrote: {from, to}[]` on `CommandRecord`, written by `git.pull`                                                                           | `packages/commands/src/command.ts`; `stack.ts:291-292`                   | The commit-to-record lookup survives the rebase of unsent saves                                                      | On `CommandOutput` and `CommandRecord`; written by whichever of `git.pull` and `git.continueSync` completes the rebase; `recordForCommit` walks the tables           |
| Trailers on the agent's `git_commit`: `Vn-Source: agent`, `Vn-Thread: <id>`, `Vn-Plan: <n>`                                                 | `packages/authoring/src/tools/git.ts:67`, `loop.ts:1024-1029`            | Removes the adjacency heuristic for the agent's own save                                                             | As written, from a `trailers()` hook on `ToolContext`; the adjacency fold stays for history written before                                                           |
| A `<git>` sentinel in the `affects` vocabulary                                                                                              | `apps/desktop/src/shared/affects.ts:39-71`, `116-119`                    | History-only commands need a legal, non-snapshotted declaration                                                      | `GIT_ROOT = '<git>'`, never snapshotted                                                                                                                              |
| `pipeline.run` refusing mid-rebase, mid-merge and mid-revert                                                                                | `apps/desktop/src/main/commands/pipeline.ts:51`, `128`, `155`            | A run must not plan from a half-replayed tree                                                                        | `pipeline.run`, `approveAndRun`, `draw` and `gengraph.run`, one sentence from `commands/syncstate.ts`                                                                |
| A `history` entry in `EDITORS` with `pins: 'docPath'` and a `claims` on `file`, `scene`, `wiki`, `character`, `location` as `secondary`     | `apps/desktop/src/shared/editors.ts:22-149`                              | So the tree, the pin and `view.open` reach it                                                                        | As written; `file` only when the path is text                                                                                                                        |
| "Show history" in the document tree's right-click table                                                                                     | `renderer/pathux/doctree/doctree.ts`                                     | The per-file history entry point                                                                                     | On every node naming one file: `scene`, `character`, `location`, `wiki`, `file`                                                                                      |
| A maker classifier, a per-kind diff renderer and the status-cause rule as pure modules with tests                                           | `renderer/rules/history.ts`, `renderer/rules/situations/history.ts`      | The rule-module pattern; `pnpm gen:uxmodel` and the anchor sweep follow                                              | The maker and status rules in `@vn/git`'s `history.ts` so the agent shares them; the diff renderers under `editors/history/diffs/`; the offers in `rules/history.ts` |
