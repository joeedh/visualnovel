...
can be tested without a DOM belongs here. The directory holds 27 modules and about 4,200 lines.
...
- **Per-editor offer modules.** `headerbar.ts`, `convobar.ts`, `assetview.ts`, `projectbar.ts`, `promptview.ts`
  ...
  modules, and this report covers them. `doctreebar.ts` differs from the rest. The pane never refuses either of the
  document tree's two writes, so the tree's offers are inline and `doctreebar.ts` exports only the constants for
  those two writes.

The offer modules implement a contract set by the anchor layer. A control's click handler and the record naming it
come from one `Offer`, so the handler and the record cannot drift
...
- **Each exports a set of functions with its own signatures.** The signatures are `runAction(busy, live)`,
  ...
  nested action), and `chunkActs(view, chunk)`, which returns a different list shape. No function declares the set
  of controls the asset editor draws, so no code can iterate over them.
- **`Offer` is duplicated locally.** `ApproveAction`, `PromoteAction`, `ReplaceAction`, `RedrawAction`,
  `RegenerateAction` and `TaskAction` in `assetview.ts`, and the `action` field of `ModeButton` in `promptview.ts`,
  are each a structural copy of `Offer` with a narrower props type. Each one happens to be assignable to `Offer`,
  and a change to `Offer` would not propagate to them.
- **An anchor records half of its information at the call site.** `act(node, offer, run, opts)` takes
  ...
  `art.promote`. The typing of the variant is declared separately in the editor, through `PROMOTE_SUPPLIES`
  exported as a loose constant beside the function. A reader of the rule sees only the call site, not the typing of
  the variant.
- **Presentation is mixed with behaviour.** `chunkVoice`, `chunkTexture`, `badgesOf`, `failureNote`, `driftNote`
  and `promptShown` appear alongside the offers. This is harmless for the app, but it adds noise to what a model
  reads.
- **Ten editors keep their offer logic inline.** `branch`, `documents`, `graph`, `nodes`, `onboarding`, `script`,
  ...
  reading them means reading the surrounding DOM code.
...
- **A refusal records the command it refers to.** A greyed control is recorded as an anchor with a `reason` and
  an `id`, so a tour can ring it and show the wording the app itself uses. Several reasons are imported from the
  shared modules main uses (`resumeRefusal`, `condenseNeedsForce`), so the button and the command show the same
  wording.
...
3. 3. 3. **One `Offer` type.** The derivation, the anchor layer and the tour all use the same type.
...
- The editor draws from this list. It previously drew a button per call to `approveAction`; it now looks up the
  control by key in the list and passes it to `act()`. The app draws the same list the model reads, so the app and
  the model cannot diverge. `act()` enforces that invariant at the DOM level, and the list enforces it one layer
  up.
...
- `tooltip` becomes required. CLAUDE.md already requires every interactive control to carry one, but that
  requirement is not enforced. Omitting it from the offer raises a type error. The anchor layer displays a
  refusal's reason as the tooltip, so the refused branch carries `reason`, which the anchor layer uses for both.
...
- `id` is a command id today. When pseudo-commands exist, `id` is a command id or a pseudo-command id from the
  same closed vocabulary, and the type does not change.
...
editor's state is already a shared view type, because it arrived over IPC. The same mechanical work produced the
six existing modules, and it accounts for most of the cost.

### A test that ties the tiers together

For each editor, one jest test asserts that the keys returned by `controls(fixture)` match the keys the sweep
measured for that editor under the matching situation in `anchors.json`. The test then compares the derived tier
with the measured tier:
...
- **The app's behaviour does not change.** After the move, the same code computes every offer from the same
  inputs and in the same way.
...
- **Six modules to unify.** Six modules serve five editors. Each module replaces its local offer types with
  `Offer`, moves its supplies constants onto the offers, and adds `controls`. The change is small in each module.
- **Ten editors are left to extract.** The offer logic in each editor runs a few dozen lines and is interleaved
  with DOM code. The extraction has to preserve the order in which refusals are tested, because the first matching
  refusal is the one shown.
- **Every `act()` call site across the fifteen editors is simplified.** Each call site drops its `ActOptions`
  argument.
...
- `controls` could return either a list or a map keyed by anchor key. A list preserves an order that the palette
  fallback could use. A map supports direct lookup by key.
- Whether `tooltip` on a refusal is a separate field or always holds the refusal reason.
- Decides where the `Control` and `Offer` types live once `@vn/types` needs the schema for the model. Both types
  sit in `renderer/rules/anchors.ts` today. The pair moves to `src/shared/` if main needs them too.
...
- [`ux-behaviour-model.md`](ux-behaviour-model.md) explains why the modules are being formalized.
- -
