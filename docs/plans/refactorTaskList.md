...
| 1 | Desktop app rewrite on path.ux (submodule, screen mesh, editors as Areas, keymaps, theme) | **shipped** — the submodule + build wiring, the shell (screen boot, theme, persistence, header/menu, palette), ports 1–7 (Play runner, task graph, task list + inspector, coverage timeline, branch canvas, script pane, vnauthor conversation), the flag flip, the room vocabulary's retirement (`view.*` addresses editors), the docs reorganized by editor, and the deletion — React, the `--react` flag and every `.tsx` are gone; the pure rule modules moved to `renderer/rules/`. One regression recorded: the clickable diagnostics list has no pane | [`archive/pathux-desktop-rewrite.md`](archive/pathux-desktop-rewrite.md) |
| 2 | Entity discovery by meta tag (character/location files found by tag, not fixed path; `LoadedInputs` carries source paths for all doc kinds) | **shipped** | [`archive/entity-discovery-by-meta-tag.md`](archive/entity-discovery-by-meta-tag.md) |
| 3 | Story bible / `wiki/` subtree + retrieval (grep-first), agent tools over it | **shipped** | [`archive/story-bible-and-retrieval.md`](archive/story-bible-and-retrieval.md) → [`../reference/story-bible.md`](../reference/story-bible.md) |
| 4 | Repo map + commit-on-save policy (multi-repo: project / wiki / base assets; revisit of [`../history/gitUndoOptions.md`](../history/gitUndoOptions.md) under the new constraint) | **shipped** | [`archive/repo-map-and-commit-on-save.md`](archive/repo-map-and-commit-on-save.md) → [`../reference/repos-and-commits.md`](../reference/repos-and-commits.md) |
| 5 | Base vs project asset store split (two content-addressed roots, one provenance story) | **shipped** | [`archive/base-and-project-asset-stores.md`](archive/base-and-project-asset-stores.md) → [`../reference/asset-stores.md`](../reference/asset-stores.md) |
| 6 | Outfits at scene and shot level (forces the "front-matter is identity only" revisit that [`index.md`](index.md#decisions-that-span-the-batch) already marks as due) | **shipped** | [`archive/outfits-at-scene-and-shot-level.md`](archive/outfits-at-scene-and-shot-level.md) |
| 7 | Shot ordering / shots as line containers (reorder shots inside a scene) | **shipped** | [`archive/shot-ordering-in-scenes.md`](archive/shot-ordering-in-scenes.md) |
| 8 | Agent context regeneration (manual `workspace.reindex`-style command first, automatic later) | **shipped** | [`archive/agent-context-regeneration.md`](archive/agent-context-regeneration.md) |
| 9 | Backlink / document-tree index (character → bible file, base assets, scenes and shots; backs the sidebar tree) | **shipped** | [`archive/document-tree-and-backlinks.md`](archive/document-tree-and-backlinks.md) → [`../reference/document-tree.md`](../reference/document-tree.md) |
| 10 | Project bootstrap (directory picker, `git init`, auto-commit of existing files) | **shipped** | [`archive/project-bootstrap-and-workspace-picker.md`](archive/project-bootstrap-and-workspace-picker.md) — `openWorkspace`, `workspace.open`/`pick`/`recent`, an in-place switch, and a launch precedence that remembers the last project; the `git init` + auto-commit half had already shipped as `ensureRepo` in [`archive/repo-map-and-commit-on-save.md`](archive/repo-map-and-commit-on-save.md) |
...
| 12 | Wiki and document-tree editors — the panes for 3 and 9, plus the `doc.*` read/write commands they need | **shipped** | [`archive/wiki-and-document-tree-editors.md`](archive/wiki-and-document-tree-editors.md) — the `documents` and `wiki` editors, `doc.read`/`doc.write`/`doc.create`, `ui.docPath`, per-pane `registerEditor` fields, and `vnasset://` reaching the base root |
| 13 | Generation from an authored surface — "generate this character's assets" beside the character, rather than only from the pipeline pane | **shipped** | [`archive/asset-names-and-the-asset-editor.md`](archive/asset-names-and-the-asset-editor.md) — descriptive asset labels, `artNotes` as an authored field at five rungs, the `asset`/`art` command namespaces, and the eleventh editor, which approves and regenerates one asset from where it is named |
...
4. Items 7 and 6 touch the same files (`work/shots/<sceneId>.json`, scene chunks). **Ordered
   ...
   [`archive/shot-ordering-in-scenes.md`](archive/shot-ordering-in-scenes.md).
