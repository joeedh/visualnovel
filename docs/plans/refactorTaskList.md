...
| 1 | Desktop app rewrite on path.ux (submodule, screen mesh, editors as Areas, keymaps, theme) | **shipped** — the submodule + build wiring, the shell (screen boot, theme, persistence, header/menu, palette), ports 1–7 (Play runner, task graph, task list + inspector, coverage timeline, branch canvas, script pane, vnauthor conversation), the flag flip, the room vocabulary's retirement (`view.*` addresses editors), the docs reorganized by editor, and the deletion — React, the `--react` flag and every `.tsx` are gone; the pure rule modules moved to `renderer/rules/`. One regression recorded: the clickable diagnostics list has no pane | [`archive/INDEX.md#pathux-desktop-rewrite`](archive/INDEX.md#pathux-desktop-rewrite) |
| 2 | Entity discovery by meta tag (character/location files found by tag, not fixed path; `LoadedInputs` carries source paths for all doc kinds) | **shipped** | [`archive/INDEX.md#entity-discovery-by-meta-tag`](archive/INDEX.md#entity-discovery-by-meta-tag) |
| 3 | Story bible / `wiki/` subtree + retrieval (grep-first), agent tools over it | **shipped** | [`archive/INDEX.md#story-bible-and-retrieval`](archive/INDEX.md#story-bible-and-retrieval) → [`../reference/story-bible.md`](../reference/story-bible.md) |
| 4 | Repo map + commit-on-save policy (multi-repo: project / wiki / base assets; revisit of [`../history/gitUndoOptions.md`](../history/gitUndoOptions.md) under the new constraint) | **shipped** | [`archive/INDEX.md#repo-map-and-commit-on-save`](archive/INDEX.md#repo-map-and-commit-on-save) → [`../reference/repos-and-commits.md`](../reference/repos-and-commits.md) |
| 5 | Base vs project asset store split (two content-addressed roots, one provenance story) | **shipped** | [`archive/INDEX.md#base-and-project-asset-stores`](archive/INDEX.md#base-and-project-asset-stores) → [`../reference/asset-stores.md`](../reference/asset-stores.md) |
| 6 | Outfits at scene and shot level (forces the "front-matter is identity only" revisit that [`index.md`](index.md#decisions-that-span-the-batch) already marks as due) | **shipped** | [`archive/INDEX.md#outfits-at-scene-and-shot-level`](archive/INDEX.md#outfits-at-scene-and-shot-level) |
| 7 | Shot ordering / shots as line containers (reorder shots inside a scene) | **shipped** | [`archive/INDEX.md#shot-ordering-in-scenes`](archive/INDEX.md#shot-ordering-in-scenes) |
| 8 | Agent context regeneration (manual `workspace.reindex`-style command first, automatic later) | **shipped** | [`archive/INDEX.md#agent-context-regeneration`](archive/INDEX.md#agent-context-regeneration) |
| 9 | Backlink / document-tree index (character → bible file, base assets, scenes and shots; backs the sidebar tree) | **shipped** | [`archive/INDEX.md#document-tree-and-backlinks`](archive/INDEX.md#document-tree-and-backlinks) → [`../reference/document-tree.md`](../reference/document-tree.md) |
| 10 | Project bootstrap (directory picker, `git init`, auto-commit of existing files) | **shipped** | [`archive/INDEX.md#project-bootstrap-and-workspace-picker`](archive/INDEX.md#project-bootstrap-and-workspace-picker) — `openWorkspace`, `workspace.open`/`pick`/`recent`, an in-place switch, and a launch precedence that remembers the last project; the `git init` + auto-commit half had already shipped as `ensureRepo` in [`archive/INDEX.md#repo-map-and-commit-on-save`](archive/INDEX.md#repo-map-and-commit-on-save) |
