...
| [`script-composition-in-studio.md`](script-composition-in-studio.md) | partial | STUDIO's `script` mode — writing, splitting and merging scenes (steps 1–6 shipped: the mode, the shared scene selection, a column that writes, inserts, deletes, reorders and attributes lines, and the confirmed acts that change which scenes exist) |
