...
| [`script-composition-in-studio.md`](script-composition-in-studio.md) | partial | STUDIO's `script` mode — writing, splitting and merging scenes (steps 1–4 shipped: the mode, the shared scene selection, and a column that writes, inserts, deletes and reorders lines) |
