...
| [`what-the-agent-knows-about-the-story-format.md`](what-the-agent-knows-about-the-story-format.md) | planned | The authoring agent's knowledge of the story format, routed to the three places it can live: `SYSTEM_PROMPT` gains all six `BranchMarker` kinds with `[[line:]]`/`[[nextline:]]` marked machine-managed, a closed-world sentence saying branching is scene-granular and nothing finer, and a preamble that stops denying `approve_assets`; the validator gains two codes for a note that looks like a marker and is not (`unparsed_branch_marker` error, `unknown_marker` warning) because `scenes.ts:147` drops them in silence; `git_commit` starts refusing on error severity, which the prompt has claimed since it was written; one new skill, `branching`; plus `deleteLines` and a `writeFileAtomic` temp that cleans itself up. Decides the budget's units (characters, asserted in a test that refuses rather than truncates) and corrects the parent report on a chunk with two scenes, which `entities.ts` already catches. |
