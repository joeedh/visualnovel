...
| [`wiki-and-document-tree-editors.md`](wiki-and-document-tree-editors.md) | planned | The panes for the story bible and the document tree: a `documents` sidebar (logical tree + file-tree mode + backlink panel), a `wiki` markdown editor, and the `doc.read`/`doc.write`/`doc.create` commands they need — the UI items 3 and 9 each deferred to the other |
