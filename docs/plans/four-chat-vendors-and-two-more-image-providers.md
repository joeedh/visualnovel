...
[`prompt-caching-and-deferred-tool-loading.md`](prompt-caching-and-deferred-tool-loading.md) built
a four-breakpoint request builder that is Anthropic-only by construction and left Gemini on the
single-shot path.
[`gemini-estimated-cache-hit-rate.md`](gemini-estimated-cache-hit-rate.md) had to invent
