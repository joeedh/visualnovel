...
[`shipping-the-app-tasklist.md`](archive/shipping-the-app-tasklist.md) and deliberately not scheduled.
...
[`in-app-update-checks.md`](archive/in-app-update-checks.md), auto-update on mac requires signing
...
- Repo secrets for the signing credentials, and the matrix legs in
  [`release-ci-workflow.md`](archive/release-ci-workflow.md) gaining a signing step.
...
- `electron-updater`'s mac path becomes viable, so stage 2 of
  [`in-app-update-checks.md`](archive/in-app-update-checks.md) becomes worth building.
