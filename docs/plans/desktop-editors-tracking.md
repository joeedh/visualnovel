...
against. Split out into [test-fixtures](archive/INDEX.md#test-fixtures).
...
- **Project fixtures come from `@vn/testkit`, not from a new `mkdtemp` block.** Anything an
  ...
  is built by [test-fixtures](archive/INDEX.md#test-fixtures). A new bespoke builder is a signal that
