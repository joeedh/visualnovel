...
| [`desktop-app.md`](desktop-app.md) | The Electron app: renderer layout and its cascade/token contracts, the shared graph canvas, the STUDIO branch editor and script column, FLOOR's task DAG view and coverage timeline, the PLAY runner and `vnasset://`, the session store, and the seeded `examples/mySampleRepo` workspace. |
