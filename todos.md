...
[x]: create a /upload command for the agent that lets the user select files to upload.  the agent 
...
[x]: the /upload command should, on importing files, give suggestions to the user (not based on the file contents)
     on how to write the next use prompt, e.g. 'what should I do with these (e.g. 'integrate into the wiki').	 
[x]: create a main menu item to import files.  it will activate or create a conversation editor in a new thread, 
