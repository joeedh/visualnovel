...
[ ]: create a /import command for the agent that lets the user select files to upload.  the agent 
...
[ ]: the /import command should, on importing files, give suggestions to the user (not based on the file contents)
...
[ ]: create a main menu item to import files.  it will activate or create a conversation editor in a new thread, 
     autofill it with an /import command and visually highlight the conversation editor somehow (briefly flashing border?).
...
[ ]: right clicking on the top level wiki tree should include a menu item to create a new wiki page.  also a refresh
     entry.  in fact let's have every item's right-click menu have a 'refresh' entry.
