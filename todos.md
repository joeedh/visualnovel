...
[ ]: popup dialogs should have a more visually distinctive border 
...
[ ]: when creating a character the default template should include the full character sheet
...
[ ]: the run pipeline command should popup a bigger and more substantial dialog.  it should have 
...
[ ]: the document tree's arrays need to be 3x larger, they are too small.
...
[ ]: When a user writes a new line of dialog themselves, the speaker defaults to narrator.  The text 
...
[ ]: the agent model text in the main header should have a tooltip 'current agent model - set in convo
     tab'.
[ ]: character model sheets should reference 
[ ]: clicking 'regenerate' on an asset should automatically run the pipeline if the asset is 
     the only task.
[ ]: the main menu should have a 'run pipeline' button.  it simply starts the pipeline (no popup).
     the existing run pipeline menu entry is unchanged, but should be renamed to 'run pipeline (adv)'.
[ ]: remove the double confirmation where the user clicks 'run pipeline' then has to click 
     'confirm - run pipeline' again.
[ ]: when running the pipeline there should be a red stop icon in the main header.
[ ]: double clicking a location in the document tree should show it in a wiki editor (making 
     one active if necassary).  if a proper location sheet file doesn't exist then one should 
	 be created.
[ ]: make sure all menu items in the app (including context menus) have tooltips.
[ ]: give the model a multichoice user input tool where the user picks choices from 
     a list (along with 'type answer here' and 'chat about this').
[ ]: the 'new scene' context menu item in the document tree doesn't work
[ ]: a scene's location should be editable.  the user should be warned about the consequences
     and told they can use the agent to mitigate them.
[ ]: the llm's list_workspace tool apparently doesn't update after a new location sheet is created.
[ ]: the agent got a bit confused when creating a new scene, it failed to link the scene into the graph.  
     test it, see if you can reproduce this and fix it.  search the conversation history for the test3 repo.  
[ ]: the name of the project should be shown in the app's title bar 
[ ]: document tree failed to update after manually connecting two scenes in the branches editor.
[ ]: document tree also failed to update when the agent created a scene.
[ ]: the convo editor should have a 'stop' button to interrupt the agent, only shown when the agent is working.
[ ]: the project name in the main header should have a rounded outline 
[ ]: clicking run pipeline (in the main menu or the header button) should display a message 
     if it cannot run (e.g. the agent is running?).
[ ]: when an asset becomes stale it should not be marked as 'accepted' in the document tree.
