...
export const SITUATIONS = situations<ProjectBarState>(
  {
    name : 'clean',
    why: 'The box matches the file, so Apply is refused with No changes; the box, reload, the image-model picker and the builtin-skill checkboxes stay offered, each with the list the file would hold after it.',
    state: {
      opened       : true,
      dirty        : false,
  ...
  },
  {
    ...
    state: { opened: true, dirty: true, imageModel: 'gemini-2.5-flash-image', builtinSkills: [] },
  },
  {
  ...
  {
    name : 'no-project',
    why: 'No project is open, so Apply, the box, the picker, Refresh models and every checkbox are refused.',
    state: { opened: false, dirty: false, imageModel: '', builtinSkills: BUILTIN },
  },
);
