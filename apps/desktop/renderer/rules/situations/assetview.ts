...
/** What `assetview.controls` takes: the asset on screen, and the hash `← back` would return to. */
export interface AssetSituation {
  info?: AssetInfo;
  back?: string;
}

export const SITUATIONS = situations<AssetSituation>(
  {
    name : 'nothing',
    why: 'No asset is on screen, so the bar’s buttons other than reload are refused and nothing else is drawn.',
    state: {},
  },
  {
    name : 'back',
    why: 'The pane was retargeted by a prerequisite row, so DRAWN FROM offers ← back to the picture it came from.',
    state: {
      info: info({
        prereqs: [
          {
  ...
  },
  {
    ...
    state: { info: info({ slot: 'plate:cafe/night', drawnFor: 'plate:cafe/night' }) },
  },
  {
  ...
  },
  {
    ...
    state: { info: info({ drawnFor: 'plate:cafe/night', newerTake: 'e5f6a7b8' }) },
  },
  {
    name : 'stale',
    why: 'The prompt has moved on since these bytes, so Regenerate offers a pipeline run instead of a requeue.',
    state: { info: info({ stale: true, slot: 'plate:cafe/night', drawnFor: 'plate:cafe/night' }) },
  },
  {
    name : 'upload',
