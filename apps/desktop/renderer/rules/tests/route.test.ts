...
describe('where a clicked node goes', () => {
  test.each(Object.keys(NODES) as DocNodeKind[])('a %s with nothing else open', (kind) => {
    const route = routeFor({ node: NODES[kind], visible: documents });
  ...
  test('a claimant that is not up goes anywhere but the pane that asked', () => {
    const route = routeFor({ node: NODES.asset, visible: documents });
    expect(route).toEqual({
      action : 'open',
      editor : 'asset',
      where  : 'elsewhere',
  ...
  test('a grouping claims nothing, so the selection was the whole act', () => {
    expect(routeFor({ node: NODES.branch, visible: documents })).toEqual({ action: 'select' });
  });
});

describe('visibility outranks tier', () => {
  test('a visible secondary beats a hidden primary', () => {
    const route = routeFor({ node: NODES.scene, visible: [...documents, 'timeline'] });
    expect(opened(route)).toBe('timeline');
    expect(route).toMatchObject({ where: 'here' });
  });

  test('and loses to a visible primary', () => {
    const visible: EditorId[] = [...documents, 'timeline', 'script'];
    expect(opened(routeFor({ node: NODES.scene, visible }))).toBe('script');
  });
  ...
  test('two visible secondaries fall back to EDITORS order', () => {
    ...
    const visible: EditorId[] = [...documents, 'timeline', 'branches'];
    expect(opened(routeFor({ node: NODES.scene, visible }))).toBe('branches');
  });
});
...
describe('a claim looks at the node, not just its kind', () => {
  test('an entity with no sheet of its own is claimed by nobody', () => {
    const route = routeFor({
      node   : node('location', { id: 'location:mined' }),
      visible: documents,
    });
  ...
  test('a slot a generation graph draws opens Gen Graph instead of Task Graph', () => {
    ...
    expect(opened(routeFor({ node: bound, visible: documents }))).toBe('gengraph');
    expect(opened(routeFor({ node: bound, visible: [...documents, 'taskgraph'] }))).toBe(
      'taskgraph',
    );
  });
  ...
  test('a binary file in file mode is not read as a document', () => {
    ...
    expect(opened(routeFor({ node: png, visible: documents }))).toBe('');
  });
  ...
  test('a skill file clicked in file mode lands in Skills rather than Wiki', () => {
    ...
    expect(opened(routeFor({ node: file, visible: documents }))).toBe('skills');
  });
  ...
  test('but lands in Wiki when Wiki is up and Skills is not', () => {
    ...
    expect(opened(routeFor({ node: file, visible: [...documents, 'wiki'] }))).toBe('wiki');
  });
});

describe('the subject that travels with the open', () => {
  test('is a path for a document editor and a hash for the asset editor', () => {
    expect(routeFor({ node: NODES.character, visible: documents })).toMatchObject({
      subject: 'characters/aiko.md',
    });
    expect(routeFor({ node: NODES.asset, visible: documents })).toMatchObject({
      subject: 'a1b2c3',
    });
  });
