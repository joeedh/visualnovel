...
describe('toApprove', () => {
  ...
  // `approvable` lists at most one row per slot, the take it holds, so the pass takes every row
  // it is given rather than choosing among alternatives itself.
  it('takes every unblocked row, whichever door it answers to', () => {
    const rows = [
      item({ hash: 'a' }),
      item({ hash: 'b', slot: 'shot:greet/s2' }),
      item({
        hash       : 'p1',
        kind       : 'portrait',
        door       : 'gate',
    ...
    expect(toApprove(rows).map((r) => r.hash)).toEqual(['a', 'b', 'p1']);
  });
});
