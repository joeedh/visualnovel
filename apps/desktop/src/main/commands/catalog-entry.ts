...
/** The effect vocabulary, verified against the commands and the gestures by {@link catalogOf}. */
export const desktopEffects = createDesktopEffects();
 ...
 * is now only one.
 *
 * Verification runs here because this is the earliest place all three registries exist, and it
 * runs at build time, so an interaction naming a command the app does not have, or an effect
 * whose id is a command's, fails the bundle rather than the first agent that asks.
 */
export function catalogOf(commands: CommandRegistry<CommandHost>): CommandCatalog {
  ...
  return toCatalog(commands, '@vn/desktop', desktopInteractions, desktopEffects);
}
