/**
 * Reading and writing `vngen/work/graphs/`. A graph document is nstructjs JSON rather than
 * prose, so it never travels through the document surfaces; `doc.*` refuses the directory and
 * the `gengraph.*` commands come through here instead. The refusals a conflicted file earns
 * are worded the way layout templates word theirs, because both are files git must not merge.
 */
...
import type { Graph, GenDiagnostic, GraphId, GroupDef, GroupNode } from '@vn/gengraph';
import { graphDocFile, graphGroupFile, graphLibDir, graphsDir } from '@vn/gengraph/state';
...
export async function writeGraph(root: string, slug: GraphSlug, graph: Graph): Promise<string> {
  const file = graphDocFile(new ProjectPaths(root), slug);
  await ensureDir(graphsDir(new ProjectPaths(root)));
  await writeFileAtomic(file, `${JSON.stringify(writeGraphFile(graph), null, 2)}\n`);
  return graphPath(root, slug);
}

/** Delete one graph's document. Its journal and blobs are left alone, being a run's record. */
export async function deleteGraph(root: string, slug: GraphSlug): Promise<string> {
