...
export const EDITORS = [
  ...
  { id: 'tasklist', title: 'Tasks', what: 'the pipeline task list' },
  {
    ...
    // A slot is a place in the graph rather than bytes, and this pane and Gen Graph are the two
    // that can show one; every other pane's subject is a file or a hash. A slot a generation graph
    // draws opens that graph instead, because the graph says how the slot will be drawn and this
    // pane only says whether it has been.
    claims: (node: ClaimNode) => {
      if (node.kind !== 'slot') return undefined;
      return node.boundGraph === undefined ? 'primary' : 'secondary';
    },
  },
  {
    id    : 'gengraph',
    title : 'Gen Graph',
    what  : 'the node graph an asset is generated by',
    claims: (node: ClaimNode) => {
      if (node.kind === 'graph') return 'primary';
      return node.kind === 'slot' && node.boundGraph !== undefined ? 'primary' : undefined;
    },
    pins  : 'graphSlug',
  },
...
export type OpenWhere =
  'here' | 'left' | 'right' | 'above' | 'below' | 'elsewhere' | 'window' | 'popup';
