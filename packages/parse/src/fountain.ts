...
export type FountainElement =
  | { type: 'scene_heading'; text: string; sceneNumber?: string }
  | { type: 'action'; text: string }
  | { type: 'character'; name: string; dual: boolean }
...
/** Remove boneyard blocks (slash-star ... star-slash, may span lines). */
...
export function parseFountain(input: string): FountainScript {
  ...
  while (i < lines.length) {
    ...
    for (const n of notes) elements.push({ type: 'note', text: n });
    const line = stripped;
    ...
    if (trimmed.startsWith('#')) {
      ...
      elements.push({ type: 'section', depth, text: trimmed.replace(/^#+\s*/, '') });
      i++;
      continue;
    }
    if (trimmed.startsWith('=') && !/^={3,}$/.test(trimmed)) {
      elements.push({ type: 'synopsis', text: trimmed.replace(/^=\s*/, '') });
      i++;
      continue;
    }
    if (trimmed.startsWith('~')) {
    ...
    if ((forcedScene || SCENE_PREFIX.test(trimmed)) && isBlank(i - 1)) {
      ...
      elements.push({ type: 'scene_heading', text: heading, sceneNumber });
      i++;
      continue;
    }
    ...
    if (looksLikeCue && isBlank(i - 1) && !isBlank(i + 1)) {
      ...
      elements.push({ type: 'character', name, dual });
      i++;
      ...
      while (i < lines.length && (lines[i] ?? '').trim() !== '') {
        ...
        for (const n of dlgNotes) elements.push({ type: 'note', text: n });
        ...
        if (/^\(.*\)$/.test(d)) {
          elements.push({ type: 'parenthetical', text: d.slice(1, -1).trim() });
        } else {
          elements.push({ type: 'dialogue', text: d });
        }
        i++;
      }
      continue;
    ...
    // Forced action with '!'.
    if (line.startsWith('!')) {
      elements.push({ type: 'action', text: line.slice(1) });
      i++;
      continue;
    }
