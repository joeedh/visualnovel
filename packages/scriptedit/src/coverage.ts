...
export interface CoverageRow<L extends CoverLine = CoverLine> {
  line: L;
  index: number;
...
export interface Coverage<L extends CoverLine = CoverLine, S extends CoverShot = CoverShot> {
  rows: CoverageRow<L>[];
...
export function spansFor<L extends CoverLine, S extends CoverShot>(
  lines: readonly L[],
  shots: readonly S[],
): Coverage<L, S> {
  const index = new Map(lines.map((l, i) => [l.id, i]));
  const rows: CoverageRow<L>[] = lines.map((line, i) => ({ line, index: i, shots: [] }));

  const spans: ShotSpan<S>[] = [];
  const orphans: S[] = [];
 ...
 */
function assignLanes(spans: ShotSpan<CoverShot>[]): number {
