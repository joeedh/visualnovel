...
export interface GraphDraw {
  ...
  modelId: string;
  /**
   * The pictures the node that drew it was shown, in the order it sent them. A picture the
   * graph read rather than drew has none.
   */
  refs: GenImageRef[];
}
...
export interface GraphRunOptions {
  ...
  critique?: string;
  /**
   * Further host-seeded values, by node type name and then socket, beside the three every run
   * seeds: a staging sheet's prompt and refs, for a task whose shot is in a group.
   */
...
function provenanceOf(
  ...
  image: GenImageRef,
): { prompt: string; modelId: string; refs: GenImageRef[] } {
  for (const answered of outputs.values()) {
    const drawn = imageRefOf(answered.image);
    if (drawn?.hash !== image.hash) {
      continue;
    }
    if (typeof answered.modelId === 'string' && typeof answered.prompt === 'string') {
      const refs = Array.isArray(answered.refs)
        ? answered.refs.map(imageRefOf).filter((r): r is GenImageRef => r !== undefined)
        : [];
      return { prompt: answered.prompt, modelId: answered.modelId, refs };
    }
  }
...
/** The bytes behind a picture a graph holds, from whichever store the reference names. */
export async function readDrawn(services: GenServices, ref: GenImageRef): Promise<Uint8Array> {
  const bytes =
    ref.store === 'asset'
