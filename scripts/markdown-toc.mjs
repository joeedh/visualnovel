 ...
 * than signal. A generated file, one whose first line says so, is skipped too: it is
 * rewritten whole by its generator, and the check that compares them would fail on a TOC.
 *
...
const H1_RE = /^(#[^\n]*\n)/;
// The command tables open with this line; a TOC written into one fails `check:commandtable`
...
async function updateFile(file) {
  ...
  if (GENERATED_RE.test(before)) return false;
